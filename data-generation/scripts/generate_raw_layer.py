...
Output: data-generation/corpus/raw/txt/APP-XXX/
...
def write_txt(app_id: str, name: str, content: str) -> None:
    ...
    print(f"  {out.relative_to(DATA_GEN.parent)}")
...
def main():
    apps = sorted(BRONZE.glob("01_application/APP-*.json"))
    total = 0
    ...
    for app_path in apps:
        ...
        # Identity
        p = BRONZE / "02_identity" / f"{app_id}_id.json"
        if p.exists():
        ...
        for seq in [1, 2]:
            p = BRONZE / "03_income" / f"{app_id}_paystub_{seq}.json"
            if p.exists():
                write_txt(app_id, f"paystub_{seq}.txt", paystub_txt(load(p), seq))
                total += 1

        # Employment verification
        p = BRONZE / "04_employment" / f"{app_id}_voe.json"
        if p.exists():
        ...
        for seq in [1, 2, 3]:
            p = BRONZE / "05_banking" / f"{app_id}_statement_{seq}.json"
            if p.exists():
                write_txt(app_id, f"bank_statement_{seq}.txt", bank_statement_txt(load(p)))
                total += 1

        # Appraisal
        p = BRONZE / "07_collateral" / f"{app_id}_appraisal.json"
        if p.exists():
            write_txt(app_id, "appraisal.txt", appraisal_txt(load(p)))
            total += 1

    print(f"\nDone — {total} files written to {RAW.relative_to(DATA_GEN.parent)}")
