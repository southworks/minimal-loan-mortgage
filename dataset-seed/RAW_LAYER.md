...
│   │       ├── bank_statement_2.txt
│   │       ├── bank_statement_3.txt
│   │       └── appraisal.txt
...
`00_raw/` is **generated output** and is listed in the repo [`.gitignore`](../.gitignore). Do not commit it.

**Source of truth:** Bronze JSON (`01_application/`, `02_identity/`, etc.) plus `generate_raw_layer.py` and `generate_agent_documents.py`.

After clone or pull, regenerate locally:
...
```bash
cd dataset-seed
python3 generate_raw_layer.py                    # 00_raw/txt/
pip install -r requirements.txt
python3 generate_agent_documents.py              # 00_raw/pdf/ and 00_raw/png/
```
