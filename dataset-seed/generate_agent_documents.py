#!/usr/bin/env python3
"""
Generate PDF and PNG agent inputs from dataset-seed Bronze JSON.

Output: dataset-seed/00_raw/pdf/APP-XXX/<category>/ and 00_raw/png/APP-XXX/<category>/
...
def generate_income(app_id: str, app: dict, formats: set[str]) -> int:
    ...
    for seq in [1, 2]:
        pay_path = BASE / "03_income" / f"{app_id}_paystub_{seq}.json"
...
def generate_employment(app_id: str, formats: set[str]) -> int:
    voe_path = BASE / "04_employment" / f"{app_id}_voe.json"
...
def generate_banking(app_id: str, formats: set[str]) -> int:
    count = 0
    for seq in [1, 2, 3]:
        stmt_path = BASE / "05_banking" / f"{app_id}_statement_{seq}.json"
...
def generate_collateral(app_id: str, formats: set[str]) -> int:
    appraisal_path = BASE / "07_collateral" / f"{app_id}_appraisal.json"
...
def generate_credit_history(app_id: str, formats: set[str]) -> int:
    credit_path = BASE / "06_credit" / f"{app_id}_credit.json"
...
def generate_for_app(app_id: str, formats: set[str]) -> int:
    app_path = BASE / "01_application" / f"{app_id}.json"
    identity_path = BASE / "02_identity" / f"{app_id}_id.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate PDF/PNG agent inputs from dataset-seed JSON.")
    ...
    if args.app:
        ...
        apps = sorted(p.stem for p in BASE.glob("01_application/APP-*.json"))

    grand_total = 0
    print(f"Generating agent inputs → {RAW.relative_to(BASE.parent)}/{{pdf,png}}")
